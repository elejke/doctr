...
__all__ = ['visualize_page', 'synthesize_page', 'synthesize_page_on_image', 'draw_boxes']
...
def extract_colors(img, x_min, x_max, y_min, y_max, n_colors=6):

    def _mc_diff(image, color):
        return np.abs((image - [[color]])).mean()

    palette = fast_colorthief.get_palette(img[x_min: x_max, y_min: y_max], quality=2, color_count=n_colors)

    img = img[:, :, :3]

    bg_c, text_c = (palette[0], palette[1]) if (_mc_diff(img, palette[0]) <
                                                _mc_diff(img, palette[1])) else (palette[1], palette[0])
...
def synthesize_page_on_image(
    ...
    """Draw a the content of the element page (OCR response) on an image.

    Args:
        page: exported Page object to represent
        image: image for drawing predictions
        draw_proba: if True, draw words in colors to represent confidence. Blue: p=1, red: p=0
        font_size: size of the font, default font = 13
        font_family: family of the font

    Return:
        the synthesized page on the image
    """
    ...
    for block in page["blocks"]:
        for line in block["lines"]:
            for word in line["words"]:
                ...
                # Draw in black the value of the word
                try:
                    d.text((0, 3), word["value"], font=font, fill=text_color)  # (0, 0, 0))
                except UnicodeEncodeError:
                    # When character cannot be encoded, use its unidecode version
                    d.text((0, 3), unidecode(word["value"]), font=font, fill=text_color)  # (0, 0, 0))
